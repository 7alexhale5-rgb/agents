...
- Viper never auto-triggers — Alex (or CMO via Alex) explicitly invokes.
...
- `KILL`: Alex either kills the artifact (delete or move to `_archive/`)
  ...
  `marketing-engine-kill-list` reopen rule. Viper does not auto-kill —
  Alex decides.
...
- Know When to Say No: Viper's job IS saying no — that's its highest
  contribution.
