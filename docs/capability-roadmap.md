...
| ------------- | ---- | ----------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------- | -------------------------------------------------------------------------- | --------- | -------------------------------------------------------------------------------------------------------------------------------------------------- |
| **atlas-ceo** | 3    | 8 (weekly-ceo-brief, weekly-ceo-operating-loop, decision-memo, source-packet-triage, business-scorecard-brief, approval-proposal-draft, self-audit, README) | 4 (`fleet.snapshot` read-only, `business.scorecard.snapshot` read-only, `atlas.propose_action` proposed_write_only, `atlas.record_follow_up` evidence_write_only) | Slack DM only         | `atlas.action.proposed` (pending) · `atlas.follow_up.recorded` (completed) | 3/day     | Live. Blind interview 9/9, legacy approval/follow-up loop passing, premium Sonnet route confirmed                                                    |
...
- **Hermes proposal-record emitter contract** — `hermes/lib/agent_events.py` (337 LOC). Reads `tools.contracts.<tool>.event` blocks, refuses to ship if missing required ADR fields (`agent_slug`, `type`, `status`, `surface`, `cwd_project`, `skill_slug`, `runtime`, `private_payload_redacted`). legacy bearer-authed PFOS POST path. Direction update: keep new work on Hermes local records/artifacts unless PFOS is explicitly reopened.
...
- **Karpathy-ladder posture** — atlas rung 3, marin/quill/stet rung 2. Falsifiable SQL gates against `Hermes local proposal records`.
...
- `hermes/lib/agent_events.py` — legacy emitter contract enforcer; do not use as the default target for new plans
...
- legacy write-back endpoints (`/api/silos/<slug>/agent-event`, etc. — separate repo; not in current plans)
...
8. **Hermes-native approval surface with audit trail** (already shipped via patch #5 emitter contract + Hermes inbox). This is what Jack doesn't have. Document it visibly so the differentiation is legible.
...
11. **Hermes inbox HITL queue.** Already shipped. The 4d-senses + LAIK + Hermes governance combo is the visible governance product.
...
16. **Hermes-record-driven workflow chaining.** Already partially built via patch #5 (legacy `agent_events` shape with `parent_run_id` field). **Extend:** Hermes reads local profile records and fires downstream profiles based on the event type. E.g., atlas-ceo emits `atlas.action.proposed` → Hermes dispatches marin to weigh in on marketing implications. The chain is recorded in `parent_run_id`.
...
18. **Plugin-based extension layer**. v0.12 ships `pre_gateway_dispatch` / `pre_approval_request` / `post_approval_response` plugin hooks. Nobody in the ecosystem has built audit-grade approval instrumentation plugins. **Build:** Hermes-aware plugin that intercepts every approval flow and writes a structured `approval_decision` event regardless of which channel the approval came from.
...
| 2    | **Wire `/goal` Ralph loop into atlas-ceo weekly-ceo-operating-loop**                       | T1    | ~2h (skill prose update + smoke test)                                                                                     | Atlas weekly-ceo-brief completes with `/goal`-tracked progress records visible in Hermes local artifacts                                               | None                                                    |
...
| 5    | **Build Hermes-aware approval-instrumentation plugin**                                       | T3    | ~8h (plugin scaffold + 3 hooks + Hermes endpoint)                                                                           | Every approval (Slack emoji + Block Kit + future) writes a structured `approval_decision` event; Hermes dashboard shows unified history | None                                                    |
...
- **`builderz-labs/mission-control` adoption** — namespace-collides with PrettyFly's retired Mission Control. Don't adopt; the Hermes approval boundary is the equivalent.
